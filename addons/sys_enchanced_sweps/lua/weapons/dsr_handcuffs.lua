...
SWEP.PrintName = "Наручники" -- change the name
        ...
        for k, v in pairs( ent.OldSWEPs ) do
            ...
            WeaponEntryButton.Paint = function( self2, w, h ) 
                ...
                if( yPos > -h ) then
                    ...
                    draw.SimpleText( "Конфисковать", "BES_Calibri_16", Spacing+((w-Spacing)/2), yPos+((h-Spacing)/2), BES.CONFIG.Themes.Text, 1, 1 )
                    surface.SetAlphaMultiplier( 1 )
                end
            end
    ...
    hook.Add( "HUDPaint", "BESHooks_HUDPaint_DrawCuffed", function()
        if( IsValid( LocalPlayer():GetActiveWeapon() ) and LocalPlayer():GetActiveWeapon():GetClass() == "dsr_handcuffs" ) then
            ...
            if( self:GetCuffTime() != 0 and self:GetTarget() ) then
                ...
                draw.DrawNonParsedSimpleText( "Надеваем", "BES_Myriad_24", ScrW()/2, ScrH()/2, Color(255, 255, 255, 255), 1, 1)
            end

            if( BES.CONFIG.HandCuffs.ShowHint ) then
                draw.SimpleText( "Левая кнопка мыши для ареста, клавиша R для обыска", "BES_Calibri_19", ScrW()/2, ScrH()-25, BES.CONFIG.Themes.Text, 1, TEXT_ALIGN_BOTTOM )
            end
            ...
            for k, v in pairs( ents.FindInSphere( LocalPlayer():GetPos(), 300 ) ) do
                ...
                if( Distance < (300*300) ) then
                    ...
                    surface.SetAlphaMultiplier( AlphaMulti )
                        draw.SimpleText( "Заперт", "BES_Myriad_38", pos2d.x, pos2d.y, Color( 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM )
                    surface.SetAlphaMultiplier( 1 )
                end
            end
        end
