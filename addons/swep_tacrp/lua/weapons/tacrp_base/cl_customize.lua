...
function SWEP:CreateCustomizeHUD()
    ...
    bg.Paint = function(self2, w, h)
        ...
        surface.SetFont("TacRP_Comfortaa_Bold_32")
    ...
    if !self:GetValue("NoRanger") then
        ...
        ranger.Paint = function(self2, w, h)
            ...
            if mouse_x > 0 and mouse_x < w and mouse_y > 0 and mouse_y < h then
                ...
                surface.SetFont("TacRP_Comfortaa_Bold_8")
                surface.SetTextColor(255, 255, 255)
    ...
    desc_box.Paint = function(self2, w, h)
        ...
        for i, k in pairs(txt) do
            surface.SetFont("TacRP_Comfortaa_8")
            surface.SetTextColor(255, 255, 255)
            surface.SetTextPos(ScreenScale(6), ScreenScale(4 + 8 + 1) + (ScreenScale(8 * (i - 1))))
            surface.DrawText(k)
        end
    end
    ...
    local stats = {
        ...
        {
            Name = "На предохранитель",
            Value = "AimDownSightsTime",
            Unit = "s",
            LowerIsBetter = true,
        },
        {
            Name = "Время перезарядки",
            AggregateFunction = function(base, val)
    ...
    for slot, attslot in pairs(self.Attachments) do
        ...
        slot_name:SetPos(airgap, airgap - ScreenScale(8) + ((slot - 1) * ScreenScale(24 + 8)))
        ...
        for i, att in pairs(atts) do
            ...
            slot_panel:SetPos(airgap + ((i - 1) * ScreenScale(24)), airgap + ((slot - 1) * ScreenScale(24 + 8)))
            slot_panel:SetSize(ScreenScale(24), ScreenScale(24))
            ...
            slot_panel.Paint = function(self2, w, h)
                ...
                surface.SetFont("TacRP_Comfortaa_4")
